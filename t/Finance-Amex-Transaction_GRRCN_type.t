...

{
  ...
  open my $fh, '<', $file_v1 or die "cannot open GRRCN file: $!";
...
{
  # set to CSV
  my $grrcn = Finance::AMEX::Transaction->new(file_type => 'GRRCN', file_format => 'CSV');
  open my $fh, '<', $file_v1 or die "cannot open GRRCN file: $!";
...
{
  # set 1.01
  my $grrcn = Finance::AMEX::Transaction->new(file_type => 'GRRCN', file_version => 1.01);
  open my $fh, '<', $file_v1 or die "cannot open GRRCN file: $!";
...
}
{
  # autodetect 2.01
  my $grrcn = Finance::AMEX::Transaction->new(file_type => 'GRRCN');
  open my $fh, '<', $file_v2 or die "cannot open GRRCN file: $!";
...
{
  # set 2.01
  my $grrcn = Finance::AMEX::Transaction->new(file_type => 'GRRCN', file_version => 2.01);
  open my $fh, '<', $file_v2 or die "cannot open GRRCN file: $!";
